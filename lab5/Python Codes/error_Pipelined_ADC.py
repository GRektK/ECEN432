...
def ADC(Vin, Vfs):
    comparators=[]
    for _ in range(3):
        comparators.append([np.random.uniform(-0.05,0.05,6),
                            np.random.uniform(-0.05,0.05,6),
                            np.random.uniform(-0.05,0.05,6),
                            np.random.uniform(-0.05,0.05,6)])
    
    RA=[]
    for _ in range(3):
        RA.append(np.random.uniform(-0.05,0.05,4))
...
decimal_codes = [ADC(Vin_i, 1) for Vin_i in Vin]
...
Vout = [ADC(Vin_i, 1)/126 for Vin_i in Vin]
